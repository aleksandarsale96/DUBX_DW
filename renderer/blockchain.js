...
class Blockchain {
  ...
  prepareTokenTransaction(
    ...
    web3Local.eth.personal.unlockAccount(
      fromAddress,
      password,
      function (error, result) {
        if (error) {
          ...
          web3Local.eth.getTransactionCount(
            fromAddress,
            "pending",
            function (error, result) {
              if (error) {
                ...
                let myContract = new web3Local.eth.Contract(
                  abi,
                  contractAddress
                );
  ...
  sendTransaction(rawTransaction, clbError, clbSuccess) {
    web3Local.eth.sendSignedTransaction(
      rawTransaction,
      function (error, result) {
        if (error) {
          clbError(error);
  ...
  getAccountsData(clbError, clbSuccess) {
    ...
    function updateBalance(index) {
      web3Local.eth.getBalance(
        rendererData.addressData[index].address,
        function (error, balance) {
          rendererData.addressData[index].balance = parseFloat(
            web3Local.utils.fromWei(balance, "ether")
          ).toFixed(2);
          rendererData.sumBalance =
            rendererData.sumBalance +
            parseFloat(web3Local.utils.fromWei(balance, "ether"));
          //   updateBinarBalance(counter);
          ...
          if (counter < rendererData.addressData.length - 1) {
            ...
            rendererData.sumBalance = parseFloat(
              rendererData.sumBalance
            ).toFixed(2);
            clbSuccess(rendererData);
          }
  ...
  importFromPrivateKey(privateKey, keyPassword, clbError, clbSuccess) {
    web3Local.eth.personal.importRawKey(
      privateKey,
      keyPassword,
  ...
  subsribePendingTransactions(clbError, clbSuccess, clbData) {
    this.txSubscribe = web3Local.eth
  ...
  subsribeNewBlockHeaders(clbError, clbSuccess, clbData) {
    this.bhSubscribe = web3Local.eth
