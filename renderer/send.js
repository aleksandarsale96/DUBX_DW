...
class SendTransaction {
    ...
    enableSendButtonTooltips() {
        DUBXUtils.createToolTip("#btnAddToAddressBook", "Add this Address to AddressBook");
        DUBXUtils.createToolTip("#btnLookForToAddress", "Look for Existing Address");
    }
    ...
    validateSendForm() {
        if (DUBXMainGUI.getAppState() == "send") {
            ...
            if (!DUBXBlockchain.isAddress($("#sendToAddress").val())) {
                DUBXMainGUI.showGeneralError("Recipient address must be a valid address!");
                return false;
            }
            ...
            if (Number($("#sendAmmount").val()) > Number($("#tokenBalance").val())) {
                DUBXMainGUI.showGeneralError("Send amount must be less then the total balance! Don't forget to leave some DUBX for fee.");
                return false;
            }
            return true;
        } else {
...
$(document).on("render_send", function () {
    ...
    $("#btnLookForToAddress").off('click').on('click', function () {
        DUBXBlockchain.getAddressListData(
            ...
            function (addressList) {
                ...
                DUBXMainGUI.renderTemplate("addresslist.html", addressList, $("#dlgAddressListBody"));
                ...
                $('#addressListFilter').off('input').on('input', function (e) {
                    DUBXUtils.filterTable($("#addressTable"), $('#addressListFilter').val());
                });
    ...
    $("#btnAddToAddressBook").off('click').on('click', function () {
        if (DUBXBlockchain.isAddress($("#sendToAddress").val())) {
            ...
            function doAddAddressToAddressBook() {
                DUBXAddressBook.setAddressName($("#sendToAddress").val(), $("#inputAddressName").val());
    ...
    $("#btnSendTransaction").off('click').on('click', function () {
        if (DUBXSend.validateSendForm()) {
            ...
            DUBXBlockchain.getTranasctionFee($("#sendFromAddress").val(), contractAddress, $("#sendAmmount").val(),
                function (error) {
                    DUBXMainGUI.showGeneralError(error);
                },
                function (data) {
                    ...
                    function doSendTransaction() {
                        ...
                        if (tokenTransaction) {
                            DUBXBlockchain.prepareTokenTransaction(
                                ...
                                function (data) {
                                    DUBXBlockchain.sendTransaction(data.raw,
                                        function (error) {
                                            DUBXMainGUI.showGeneralError(error);
                                        },
                                        function (data1) {
                                            ...
                                            DUBXBlockchain.getTransaction(data1,
                                                function (error) {
                                                    DUBXMainGUI.showGeneralError(error);
                                                },
