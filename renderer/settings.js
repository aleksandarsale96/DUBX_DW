...
class Settings {
    ...
    renderSettingsState() {
        DUBXMainGUI.renderTemplate("settings.html", {});
...
$(document).on("render_settings", function() {
    $("#btnSettingsCleanTransactions").off('click').on('click', function() {
        if (isFullySynced) {
            DUBXMainGUI.showGeneralConfirmation("Do you really want to resync transactions?", function(result) {
                if (result) {
                    if (DUBXTransactions.getIsSyncing()) {
                        DUBXMainGUI.showGeneralError("Transactions sync is currently in progress");
                    } else {
                        // first disable keepInSync
                        DUBXTransactions.disableKeepInSync();
                        ...
                        if (ipcResult.success) {
                            ...
                            web3Local.eth.getBlock("latest", function(error, localBlock) {
                                if (error) {
                                    ...
                                    DUBXTransactions.syncTransactionsForAllAddresses(localBlock.number);
                            ...
                            DUBXMainGUI.showGeneralError("Error resyncing transactions: " + ipcResult.error);
                        }
                    }
                }
            });
    ...
    $("#btnSettingsCleanWallets").off('click').on('click', function() {
        DUBXMainGUI.showGeneralConfirmation("Do you really want to delete wallets data?", function(result) {
            if (result) {
                ...
                if (ipcResult.success) {
                    ...
                    DUBXMainGUI.showGeneralError("Error clearing wallet names: " + ipcResult.error);  
                }
            }
        });
    });
    ...
    $("#btnSettingsCleanBlockchain").off('click').on('click', function() {
        DUBXMainGUI.showGeneralConfirmation("Do you really want to delete the blockchain data? Wallet will close and you will need to restart it!", function(result) {
            if (result) {                
                ...
                setTimeout(() => {
                    ...
                    ipcResult = ipcRenderer.send('stopGdubx', null);
