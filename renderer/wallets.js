...
class Wallets {
  ...
  validateNewAccountForm() {
    if (DUBXMainGUI.getAppState() == "account") {
      ...
      if (
        $("#walletPasswordFirst").val() !== $("#walletPasswordSecond").val()
      ) {
  ...
  renderWalletsState() {
    ...
    DUBXBlockchain.getAccountsData(
      ...
      function (data) {
        ...
        $("#labelSumDollars").html(
          vsprintf("/ %.2f $ &nbsp;&nbsp;&nbsp;&nbsp;Price DUBX/USD %.4f $", [
            data.sumBalance * DUBXWallets._getPrice(),
            DUBXWallets._getPrice(),
          ])
        );
      }
    );
    ...
    .on("click", function () {
      ...
      function doCreateNewWallet() {
        ...
        if (DUBXWallets.validateNewAccountForm()) {
          DUBXBlockchain.createNewAccount(
            $("#walletPasswordFirst").val(),
            function (error) {
              DUBXMainGUI.showGeneralError(error);
            },
      ...
      $("#btnCreateWalletConfirm")
        .off("click")
        .on("click", function () {
          doCreateNewWallet();
        });

      $("#dlgCreateWalletPassword")
        .off("keypress")
    ...
    .on("click", function () {
      ...
      function doImportFromPrivateKeys() {
        ...
        if (DUBXWallets.validateImportFromKeyForm()) {
          var account = DUBXBlockchain.importFromPrivateKey(
            $("#inputPrivateKey").val(),
            $("#keyPasswordFirst").val(),
            function (error) {
              DUBXMainGUI.showGeneralError(error);
            },
            function (account) {
              if (account) {
                ...
                DUBXMainGUI.showGeneralError(
                  "Error importing account from private key!"
                );
              }
            }
          );
      ...
      $("#btnImportFromPrivateKeyConfirm")
        .off("click")
        .on("click", function () {
          doImportFromPrivateKeys();
        });

      $("#dlgImportFromPrivateKey")
        .off("keypress")
        .on("keypress", function (e) {
          if (e.which == 13) {
            doImportFromPrivateKeys();
