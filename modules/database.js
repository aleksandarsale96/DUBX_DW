  ...
  db.find({}).exec(function (err, docs) {
    ...
    docs.sort((a, b) => {
      if (!b.block && a.block) {
        ...
        return (
          moment(b.timestamp, "YYYY-MM-DD HH:mm:ss").toDate() -
          moment(a.timestamp, "YYYY-MM-DD HH:mm:ss").toDate()
        );
      } else {
...
ipcMain.on("deleteBlockchainData", (event, arg) => {
  ...
  function getBlockchainDataLocation() {
    switch (os.type()) {
      case "Darwin":
        return path.join(os.homedir(), "Library", "Dubxcoin", "gdubx");
      case "Linux":
        return path.join(os.homedir(), ".dubxcoin", "gdubx");
      default:
        return path.join(process.env.APPDATA, "dubxcoin", "gdubx");
    }
  }
